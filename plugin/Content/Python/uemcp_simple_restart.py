...
def simple_restart():
    ...
    if 'uemcp_listener' in sys.modules:
        ...
        if hasattr(mod, 'server_running') and mod.server_running:
            ...
            unreal.log("      import importlib; importlib.reload(uemcp_listener); uemcp_listener.start_listener()")
        else:
            unreal.log("UEMCP: Listener not running. To start:")
            unreal.log("      import uemcp_listener; uemcp_listener.start_listener()")
    else:
