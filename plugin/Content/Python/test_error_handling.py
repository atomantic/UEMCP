...
def test_error_handling_framework():
    ...
    try:
        result = ops.spawn(assetPath="invalid_path", location=[0, 0, 100])  # Should fail validation
    ...
    try:
        result = ops.spawn(assetPath="/Engine/BasicShapes/Cube", location=[0, 0])  # Should fail - need 3 elements
    ...
    try:
        result = ops.spawn(assetPath="/Game/NonExistentAsset", location=[0, 0, 100])  # Valid format, but doesn't exist
    ...
    try:
        result = ops.spawn(
            assetPath="/Engine/BasicShapes/Cube", location=[100, 100, 100], name="ErrorHandlingTest_Cube"
        )
        ...
        if result.get("success", True):  # New framework defaults to success
            ...
            if delete_result.get("success", True):
                print("✅ Successfully deleted test actor")
        else:
