...
export const levelActorsTool = {
  definition: {
    name: 'level_actors',
    description: 'List actors in the current level with their properties. Examples: level_actors({}) for all actors, level_actors({ filter: "Wall" }) for actors with "Wall" in name, level_actors({ filter: "Light", limit: 10 }) for first 10 lights',
    inputSchema: {
      type: 'object',
      properties: {
        filter: {
